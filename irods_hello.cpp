...
extern "C" {
    ...
    int irods_hello(msParam_t *inPath, msParam_t *inAttr, msParam_t *inVal,
                    msParam_t *inUnit, msParam_t* _out, ruleExecInfo_t* _rei ) {
    ...
    irods::ms_table_entry*  plugin_factory() {
        ...
        //    the first param is the name / key of the operation, the second
        //    is the name of the function which will be the microservice
        msvc->add_operation( "irods_hello2", "irods_hello2" );
    ...
    // =-=-=-=-=-=-=-
    // 1. Write a standard issue microservice
    int irods_hello2(msParam_t *inPath, msParam_t *inAttr, msParam_t *inVal,
                    msParam_t *inUnit, msParam_t* _out, ruleExecInfo_t* _rei ) {
        std::string my_str = parseMspForStr(inAttr);
        ...
        unitStr = parseMspForStr (inUnit);
        outStr = (char*)malloc(MAX_NAME_LEN);
        snprintf(outStr, MAX_NAME_LEN, "Hello World 2!\nOur input string was: %s %s %s %s", pathStr, attrStr, valStr, unitStr);
